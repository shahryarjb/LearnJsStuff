...
`react-window` لایبرری هست که  که کمک میکنه لیست های بزرگ و طولانی به طور بهینه رندر بشوند
...
`react-window` یک کتابخانه کوچک است که ایجاد لیست‌های مجازی‌سازی شده در برنامه را آسان‌تر می‌کند. این کتابخانه تعدادی API پایه را ارائه می‌دهد که می‌توان از آن‌ها برای انواع مختلفی از لیست‌ها و جداول استفاده کرد.

#### fixed size lists (لیست با طول ثابت) در چه زمانی استفاده کنیم؟
...
`react-window` همچنین پشتیبانی از مجازی‌سازی لیست‌های چندبعدی یا گریدها را فراهم می‌کند. در این زمینه، "پنجره" محتوای قابل مشاهده با اسکرول کردن کاربر به صورت افقی و عمودی تغییر می‌کند.
...
- `isItemLoaded`: متدی که بررسی می‌کند آیا آیتم خاصی بارگذاری شده است
- `itemCount`: تعداد آیتم‌ها در لیست (یا تعداد مورد انتظار)
- `loadMoreItems`: کال‌بکی که یک پرامیس برمی‌گرداند که به داده‌های اضافی برای لیست منجر می‌شود
...
4. اگر نیاز به بارگذاری تنبل آیتم‌ها همانطور که کاربر اسکرول می‌کند دارید، لیست مجازی‌سازی شده خود را با `react-window-infinite-loader` بپوشانید.
5. برای لیست‌های خود از ویژگی `overscanCount` و برای گریدهای خود از ویژگی‌های `overscanColumnsCount` و `overscanRowsCount` استفاده کنید تا از نمایش فضای خالی جلوگیری کنید. بیش از حد ورودی‌ها را اسکن نکنید زیرا این کار بر عملکرد به طور منفی تأثیر می‌گذارد.
